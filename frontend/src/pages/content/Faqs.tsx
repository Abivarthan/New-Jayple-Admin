...
export const Faqs: React.FC = () => {
  ...
  const handleSave = async () => {
    if (!editing?.question || !editing?.answer) return toast.error('Question and answer are required');
    try {
      setIsSaving(true);
      if (editing.id) {
  ...
  if (isLoading) return <div className="p-8 text-gray-500">Loading FAQs...</div>;
    ...
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Frequently asked questions shown in the customer app. Live on save.</p>
        </div>
        <button
      ...
      {!isEditing ? (
        <div className="space-y-3">
          {items.map((f) => (
            <div key={f.id} className="flex items-start gap-4 p-4 bg-white border border-gray-200 rounded-xl">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => { setEditing(f); setIsEditing(true); }} className="p-2 text-gray-500 hover:text-black font-semibold bg-slate-700 rounded-lg transition-colors">
                  <Edit2 size={16} />
                </button>
                <button onClick={() => handleDelete(f.id)} className="p-2 text-gray-500 hover:text-rose-400 bg-slate-700 rounded-lg transition-colors">
                  <Trash2 size={16} />
                </button>
              </div>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
          <h2 className="text-lg font-bold text-gray-900">{editing?.id ? 'Edit FAQ' : 'Add FAQ'}</h2>
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">Question</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">Answer</label>
            <textarea
              rows={5}
          ...
          <div className="flex items-center gap-3">
            ...
            />
            <label htmlFor="faqActive" className="text-sm font-medium text-gray-800">FAQ is Active</label>
          </div>
