  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900">Refund Cases</h1>
          <p className="text-sm text-gray-500">Vendor-rejected bookings awaiting a manual refund. Pay the customer's online/card
            portion, then mark it refunded — the wallet portion is credited automatically.</p>
        </div>
        <button onClick={load} disabled={loading}
          className="flex items-center gap-2 rounded-lg border border-gray-200 bg-slate-700 hover:bg-slate-600 text-gray-900 px-4 py-2.5 text-sm font-semibold disabled:opacity-50">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500 uppercase font-semibold">Pending Refunds</span>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500 uppercase font-semibold">Amount Owed to Customers</span>
            <div className="rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/20 p-1.5"><IndianRupee size={14} /></div>
          </div>
          <h3 className="text-2xl font-bold text-gray-900 mt-2">₹{pendingTotal.toLocaleString()}</h3>
        </div>
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <div className="flex items-center justify-between">
            <span className="text-xs text-gray-500 uppercase font-semibold">Processed</span>
          ...
          <h3 className="text-2xl font-bold text-gray-900 mt-2">{rows.length - pendingCount}</h3>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3 bg-white p-4 rounded-xl border border-gray-200">
        <div className="relative sm:col-span-2">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
            ...
            className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2.5 pl-9 pr-4 text-xs text-gray-900 placeholder-slate-500 outline-none focus:border-black" />
        </div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value as typeof statusFilter)}
          className="w-full rounded-lg border border-gray-200 bg-gray-50 py-2.5 px-3 text-xs text-gray-500 outline-none focus:border-black cursor-pointer">
      ...
      <div className="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-xl">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-700 text-sm text-gray-800">
            {filtered.map((r) => (
              <tr key={r.id} className={`hover:bg-gray-50/10 ${r.refundStatus === 'pending_admin' ? 'bg-amber-500/[0.03]' : ''}`}>
                <td className="py-4 px-6">
                  <span className="font-semibold text-gray-900 block">{r.customerName}</span>
                  {r.customerPhone && (
                    <a href={`tel:${r.customerPhone}`} className="text-[11px] text-black font-semibold flex items-center gap-1 mt-0.5">
                      <Phone size={10} /> {r.customerPhone}
                    </a>
                  )}
                </td>
                <td className="py-4 px-6">
                  <span className="font-medium text-gray-800 block">{r.vendorName}</span>
                  <span className="text-[10px] text-rose-400">Rejected by vendor</span>
                </td>
                <td className="py-4 px-6">
                  <span className="text-xs text-gray-500 block">{r.serviceNames}</span>
                  <span className="text-[10px] text-gray-500">{r.slotDate} {r.slotTime} · #{r.id.slice(-6).toUpperCase()}</span>
                </td>
                <td className="py-4 px-6 text-right">
                  <div className="text-xs font-bold text-gray-900">₹{r.refundAmount.toLocaleString()}</div>
                  <div className="text-[10px] text-gray-500 flex items-center justify-end gap-2 mt-0.5">
                ...
                <td className="py-4 px-6 text-right">
                  {r.refundStatus === 'pending_admin' ? (
                    <button onClick={() => setConfirming(r)}
                      className="rounded bg-emerald-600 hover:bg-emerald-500 text-gray-900 text-xs font-semibold py-1.5 px-3.5">
                      Mark Refunded
                    </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 shadow-2xl space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-900">Mark refund processed?</h3>
                <p className="text-xs text-gray-500 mt-0.5">Confirm you have paid the online/card portion to the customer.</p>
              </div>
            </div>
            <div className="rounded-lg border border-gray-200 bg-gray-50/40 p-4 text-xs text-gray-800 space-y-1.5">
              <div className="flex justify-between"><span className="text-gray-500">Customer</span><span className="font-semibold">{confirming.customerName} · {confirming.customerPhone || '—'}</span></div>
              <div className="flex justify-between"><span className="text-gray-500 flex items-center gap-1"><CreditCard size={11} /> Pay manually (online/card)</span><span className="font-bold text-gray-900">₹{confirming.refundOnlineAmount}</span></div>
              <div className="flex justify-between"><span className="text-gray-500 flex items-center gap-1"><Wallet size={11} /> Auto-credited to wallet</span><span className="font-bold text-emerald-400">₹{confirming.refundWalletAmount}</span></div>
              <div className="flex justify-between border-t border-gray-200 pt-1.5 mt-1"><span className="font-semibold text-gray-800">Total refund</span><span className="font-extrabold text-gray-900">₹{confirming.refundAmount}</span></div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 border-t border-gray-200 pt-4">
              <button onClick={() => setConfirming(null)} disabled={busy}
                className="rounded-lg border border-gray-200 bg-gray-50/20 text-gray-500 hover:bg-gray-100 text-xs font-semibold py-2 px-4 disabled:opacity-40">Cancel</button>
              <button onClick={doRefund} disabled={busy}
                className="flex items-center gap-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-xs font-semibold text-gray-900 py-2 px-4 disabled:opacity-50">
